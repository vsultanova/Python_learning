...
def main():
    ...
    # Autogenerate News or Adv file to parse data from
    ...
    # Autoremove the file with test data to parse
    remove_processed_file = RemoveFile()

    # Test data and file paths
    file_with_output = "HW10_output.txt"
    db_name = "TestDB.db"
    ...
    if askuseroption.answer_option == 1:
        ...
        # Define the DB name, table name, columns and data
        # Create News table if not exists, insert data and retrieve data
        table_name = askuseroption.news
        col1 = "News_text"
        col2 = "News_city"
        news_text = filedata.user_list[0]
        news_city = filedata.user_list[1]
        dbcon = DBCon(db_name, table_name, col1, col2, news_text, news_city)
        dbcon.open_db()
        dbcon.create_table()
        dbcon.insert_db()
        dbcon.retrieve_data()
        ...
        # Define the DB name, table name, columns and data
        # Create Adv table if not exists, insert data and retrieve data
        table_name = askuseroption.adv
        col1 = "Adv_text"
        col2 = "Exp_date"
        adv_text = filedata.user_list[0]
        exp_date = filedata.user_list[1]
        dbcon = DBCon(db_name, table_name, col1, col2, adv_text, exp_date)
        dbcon.open_db()
        dbcon.create_table()
        dbcon.insert_db()
        dbcon.retrieve_data()
...
class DBCon:
    def __init__(self, db, table, col1, col2, name1, name2):
        self.db_name = db
        self.table = table
    ...
    def open_db(self):
        self.connection = sqlite3.connect(self.db_name)
        print(f"Opened '{self.db_name}'")
        return self.db_name

    def create_table(self):
        try:
            self.connection.execute(f"CREATE TABLE {self.table} ({self.col1} text, {self.col2} text)")
            print(f"Created '{self.table}' table")
    ...
    def close_db(self):
        ...
        print(f"'{self.db_name}' is closed and disconnected")

    def insert_db(self):
        self.connection.execute(f"INSERT INTO {self.table} VALUES ('{self.name1}', '{self.name2}')")
        self.connection.commit()
        print(f"{self.table} is inserted and committed")

    def retrieve_data(self):
        self.cursor.execute(f"SELECT * FROM {self.table}")
        self.result = self.cursor.fetchall()
        length = len(self.result)
        print(f"Table '{self.table}' is list with {length} items")
        if length > 0:
            # for row in self.result:
                # print("Fetched row:", row)
            print(f"Last row is {self.result[length-1]}")
            # print(self.result[length-1][0])
            # print(self.result[length-1][1])
